...
//   // Função para verificar se a ordem já está sendo usada
//   const isOrderAlreadyUsed = (order: number) => {
//     return items.some(item => item.order == order);
//   };
...
//     }else if(ordemValue < 1 || ordemValue > 20 ){
//       errors['ordem'] = 'A ordem deve ser um número entre 1 e 20';

//     }else if (isOrderAlreadyUsed(ordemValue)) {
...
//                     const clampedValue = Math.min(Math.max(parseInt(sanitizedValue, 10) || 0, 1), 20);

//                     if (isOrderAlreadyUsed(clampedValue)) {
